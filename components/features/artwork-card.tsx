...
export function ArtworkCard({ artwork, viewMode = 'grid' }: ArtworkCardProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group block transition-transform duration-200 hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900 dark:focus-visible:ring-zinc-300 focus-visible:ring-offset-2 rounded-lg",
        isListView && "hover:scale-100"
      )}
      aria-label={`View details for ${artwork.title} by ${artwork.artistDisplayName || 'Unknown Artist'}`}
    >
      <Card className={cn(
        "overflow-hidden h-full transition-shadow hover:shadow-lg",
